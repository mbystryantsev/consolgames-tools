...
type
  ...
  public
   ...
   // Procedure HLine(x1, x2, y1: Integer);
    ...
    property TransparentColor: Integer read FTransparentColor write FTransparentColor;
  end;
...
begin
  ...
  begin
    if Source<>Self then With TDIB(Source) do
    begin
     ...
     Self.Height := Height;
     Self.UpdatePalette;
     Move(PBits^, Self.PBits^, WidthBytes * Height);
//     Self.Canvas.Draw(0, 0, TDIB(Source));
    end;
    ...
    begin
      ...
      begin
        ...
        case BitCount of
          ...
          32: begin
                PDWORD(DestP)^ := pfRGB(NowPixelFormat, cR, cG, cB) or cA shl 24;
                Inc(PDWORD(DestP));
              end;
        end;
      end;
...
procedure TCustomDXPaintBox.Paint;
  ...
  begin
    ...
    begin
      if FCenter then
      begin
       FImageLeft := -(Width-ClientWidth) div 2;
       FImageTop := -(Height-ClientHeight) div 2;
        inherited Canvas.StretchDraw(Bounds(FImageLeft,
          FImageTop, Width, Height), FDIB);
      end else
      begin
       FImageLeft := 0;
       FImageTop := 0;
    ...
    begin
      ...
      begin
       FImageLeft := -(Width-ClientWidth) div 2;
       FImageTop := -(Height-ClientHeight) div 2;
        inherited Canvas.Draw(FImageLeft, FImageTop,
          FDIB);
      end else
      begin
       FImageLeft := 0;
...
procedure TCustomDXPaintBox.SetScale(Value: Integer);
begin
 If Value < 1 then Value := 1;
 If Value > 8 then Value := 8;
 If Value = FScale then Exit;
 FScale := Value;
 If FViewWidth <= 0 then FViewWidth := DIB.Width;
 If FViewHeight <= 0 then FViewHeight := DIB.Height;
 FViewWidth := FViewWidth * FScale;
 FViewHeight := FViewHeight * FScale;
 Invalidate;
end;
...
procedure TDIB.Fill8bit(ColorIndex: Byte);
begin
 FillChar(FPBits^, FWidthBytes * FHeight, ColorIndex);
end;

procedure TDIB.Fill4bit(ColorIndex: Byte);
Var C: Byte;
begin
 C := ColorIndex and 15;
 C := C + C shl 4;
 FillChar(FPBits^, FWidthBytes * FHeight, C);
end;

Procedure TDIB.MoveTo(X, Y: Integer);
begin
 FPenX := X;
 FPenY := Y;
end;

Procedure TDIB.SetColor(Value: Integer);
begin
 FColorIndex := Value;
end;

Procedure TDIB.SetPenWidth(Value: Integer);
begin
 FPenWidth := Value;
end;

Procedure TDIB.LineTo(X, Y: Integer);
Var
 Dx, dx2, dy, dy2, Error, Eps, t, x1, y1, x2, y2: Integer;
...
begin
 If (X >= 0) and (Y >= 0) and (X < FWidth) and (Y < FHeight) then
    case FBitCount of
      1 : begin
            P := @PArrayByte(Integer(FTopPBits)+Y*FNextLine)[X shr 3];
            P^ := (P^ and Mask1n[X and 7]) or ((DWord(FColorIndex) and 1) shl Shift1[X and 7]);
          end;
      4 : begin
            P := @PArrayByte(Integer(FTopPBits)+Y*FNextLine)[X shr 3];
            P^ := (P^ and Mask4n[X and 1]) or ((DWord(FColorIndex) and 15) shl Shift4[X and 1]);
          end;
      8 : PArrayByte(Integer(FTopPBits)+Y*FNextLine)[X] := DWord(FColorIndex);
      16: PArrayWord(Integer(FTopPBits)+Y*FNextLine)[X] := DWord(FColorIndex);
      24: with PArrayBGR(Integer(FTopPBits)+Y*FNextLine)[X] do
          begin
            B := Byte(DWord(FColorIndex) shr 16);
            G := Byte(DWord(FColorIndex) shr 8);
            R := Byte(DWord(FColorIndex));
          end;
      32: PArrayDWord(Integer(FTopPBits)+Y*FNextLine)[X] := DWord(FColorIndex);
    end;
end;
...
begin
 ...
 Dec(X, XX);
 Dec(Y, XX);
 For YY := 0 to FPenWidth - 1 do
  For XX := 0 to FPenWidth - 1 do
   SetPixel(X + XX, Y + YY);
end;
Procedure BigY;
begin
 ...
 If y1 < y2 then Eps := 1 Else Eps := -1;
 Error := -Dy;
 ...
 If y1 = y2 then PutPixel(x1, y1) Else
 begin
  PutPixel(x1, y1);
  PutPixel(x2, y2);
 end
...
begin
 ...
 If x1 < x2 then Eps := 1 Else Eps := -1;
 Error := -Dx;
 ...
 If x1 = x2 then PutPixel(x1, y1) Else
 begin
  PutPixel(x1, y1);
  PutPixel(x2, y2);
 end
...
begin
 ...
 Dx2 := Dx * 2;
 Dy2 := Dy * 2;
 If Dx <= Dy then BigY Else BigX;
end;

Procedure TDIB.DrawDIB(X, Y: Integer; Image: TDIB);
Var W, H, NL, ISS, TC: DWord; X1, Y1, X2, Y2: Integer; Src, Dest: DWord;
begin
 If Image.FBitCount <> FBitCount then Exit;
 X1 := X; Y1 := Y;
 x2 := x1 + Image.Width - 1;
 y2 := y1 + Image.Height - 1;
 If (X1 >= FWidthBytes) or (Y1 >= FHeight) or (X2 < 0) or (Y2 < 0) then Exit;
 Src := DWord(Addr(Image.PBits^));
 ISS := Image.WidthBytes - Image.Width;
 If x1 < 0 then
 begin
  Inc(ISS, Abs(x1));
  Inc(Src, Abs(x1));
 ...
 If x2 >= FWidthBytes then
 begin
  Inc(ISS, (x2 - (FWidthBytes - 1)));
  x2 := FWidthBytes - 1;
 end;
 If y1 < 0 then y1 := 0;
 If y2 >= FHeight then
 begin
  Inc(Src, (y2 - (FHeight - 1)) * Image.WidthBytes);
  y2 := FHeight - 1;
 end;
 H := y2 - y1 + 1;
 W := x2 - x1 + 1;
 NL := DWord(FWidthBytes) - W;
 Dest := DWord(Addr(FPBits^));
 Inc(Dest, ((DWord(FHeight) - 1) - (DWord(Y1) + (H - 1))) * DWord(FWidthBytes) + DWord(X1));
 TC := Image.TransparentColor;
 If not Image.Transparent then
 begin
  If Image.BitCount = 8 then
  Asm
   Mov ESI, Src
   Mov EDI, Dest
   Cld
...
procedure TDIB.LoadFromResourceName(Instance: THandle; const ResName: string);
var
