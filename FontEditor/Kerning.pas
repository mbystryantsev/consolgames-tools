...
begin
  Result := 0;
  If (Index < 0) or (Index >= FontData.Count) Then Exit;
  DrawCharData(img, Chars[Index], x + FontData.CharData[Index].L + Ker, y);
...
begin
  For i := 0 To High(FontData.CharData) do
    If FontData.CharData[i].Code = C Then
      break;
  if (i >= 0) and (i < FontData.Count) Then
    Result := i
  else
    Result := -1;
end;
...
begin
  FontData.KerningCount := Length(KerningData);
  SetLength(FontData.KerningData, FontData.KerningCount);
  Move(KerningData[0], FontData.KerningData[0], SizeOf(TKerningData) * FontData.KerningCount);
  Close();
end;
