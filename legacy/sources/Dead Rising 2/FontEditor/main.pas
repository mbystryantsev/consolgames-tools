...
Type
 ...
 TSizeData = Packed Record
  ...
  TTexHeader = Packed Record
    ...
    Width, Height: Word;
    Unk: Array[0..7] of Byte;
    Unk1, Unk2: LongWord;
    HeaderSize: LongWord;
    DataSize: LongWord;
    CommonSize: LongWord;
    Unk3: Array[0..2] of LongWord;
  end;
...
begin
  ...
  For n := 0 To High(Palette) do
    Integer(DIB.ColorTable[n]) := Palette[n];
...
begin              
 ...
 C := WideChar(FontData.CharData[CurChar].Code);
 ...
 With FontData.CharData[CurChar] do StatusBar.Panels[4].Text :=
  Format('W: %d, L: %d, Y: %d, %f, %f',[W, L, Y, Rect.Left, Rect.Right]);
end;
...
begin        
 ...
 begin
  ...
  Pen.Width := 1;
  MoveTo((FontData.CharData[CurChar].W - FontData.CharData[CurChar].L)* WW, 0);
  LineTo((FontData.CharData[CurChar].W - FontData.CharData[CurChar].L)* WW, 256);
...
begin
 FillChar(CT, 1024, 0);
 For I := 0 to 255 do With TRGBA(Palette[I]), CT[I] do
 begin
  rgbRed := R;
  rgbGreen := G;
  rgbBlue := B;
...
//const l_matrix: Array[0..7] of Integer = (0, 1, 2, 3, 4, 5, 6, 7);
//const t_matrix: Array[0..7] of Integer = (0, 1, 2, 3, 4, 5, 6, 7);
...
Procedure DecodeImage(Src, Dest: Pointer; Width, Height: Integer);
var Image, p, dp: ^Byte; Count, w, h, t, th, x, y, x1, x2, y1, y2, xx, yy, index: Integer;
DIB: TDIB;
begin
  w := RoundBy(Width, 32);
  h := RoundBy(Height, 32);
  ...
  begin
    ...
    begin
      ...
      begin
        For xx := 0 To 7 do
        begin
          PByte(LongWord(Image) + (y + l_matrix[yy] + th) * w + x + xx + ((t_matrix[t] mod 4) * 8))^ := p^;
          Inc(p);
        end;
      end;
    end;
  ...
  DIB.Height := h;
  For y := 0 To 255 do
    Integer(DIB.ColorTable[y]) := Palette[y];
  DIB.UpdatePalette;

  For y := 0 To Height - 1 do
  begin
    Move(p^, dp^, Width);
    Move(p^, DIB.ScanLine[Y]^, w);
    Inc(p, w);
    Inc(dp, Width);
  end;
...
begin
  Inc(FontData.FontHeader.Count);
  SetLength(FontData.CharData, FontData.FontHeader.Count);
  With FontData.CharData[FontData.FontHeader.Count - 1] do
  begin
    Code := LongWord('?');
    W := 0;
    //Unk2 := $41;
  end;
  If not CharPropForm.Show(FontData.CharData[FontData.FontHeader.Count - 1]) Then
  begin
    Dec(FontData.FontHeader.Count);
    SetLength(FontData.CharData, FontData.FontHeader.Count);
  end else
  begin
    Saved := False;
    CurChar := FontData.FontHeader.Count - 1;
    SetLength(Chars, FontData.FontHeader.Count);
    FillChar(Chars[CurChar], SizeOf(TCharacter), 0);
    DrawChars;
    DrawChar;
  end;
...
Function FindSection(const FontData: TFontData; const Name: String): PSectionHeader;
var i: Integer;
begin
  Result := nil;
  For i := 0 To FontData.FileHeader.SectionCount - 1 do
  begin
    If FontData.SectionNames[i] = Name Then
    begin
      Result := @FontData.Sections[i];
      Exit;
    end;
  end;
end;
...
begin
  ...
  try
    ...
    Stream.Read(FontData.FileHeader, SizeOf(TFileHeader));
    SetLength(FontData.Sections, FontData.FileHeader.SectionCount);
    SetLength(FontData.SectionNames, FontData.FileHeader.SectionCount);
    If Integer(FontData.FileHeader.Magic) = $03040506 Then
    begin
      SetLength(FontData.SectionsAlt, FontData.FileHeader.SectionCount);
      Stream.Read(FontData.SectionsAlt[0], SizeOf(TSectionHeaderAlt) * FontData.FileHeader.SectionCount);
      For i := 0 To FontData.FileHeader.SectionCount - 1 do
      begin
        FontData.Sections[i].NamePos := FontData.SectionsAlt[i].NamePos;
        FontData.Sections[i].Size := FontData.SectionsAlt[i].Size;
        FontData.Sections[i].Offset := FontData.SectionsAlt[i].Offset;
      end;
    end else
      Stream.Read(FontData.Sections[0], SizeOf(TSectionHeader) * FontData.FileHeader.SectionCount);
    For i := 0 To FontData.FileHeader.SectionCount - 1 do
    begin
      Stream.Seek(FontData.Sections[i].NamePos, 0);
      FontData.SectionNames[i] := '';
      While True Do
      begin
        Stream.Read(C, 1);
        if(C = #0) Then break;
        FontData.SectionNames[i] := FontData.SectionNames[i] + C;
      end;
    end;

    Section := FindSection(FontData, '_FONTHEADER_');
    ...
    SetLength(FontData.CharData, FontData.FontHeader.Count);
    Section := FindSection(FontData, '_FONTDATA_');
    Stream.Seek(Section^.Offset, 0);
    Stream.Read(FontData.CharData[0], SizeOf(TCharRecord) * FontData.FontHeader.Count);
    //!!
    For i := 0 To FontData.FontHeader.Count - 1 do
      //!!
      EndianChar(FontData.CharData[i]);
    

    Section := FindSection(FontData, '_TEXTURE_');
    Stream.Seek(Section^.Offset, 0);
    ...
    SetLength(Chars, FontData.FontHeader.Count);
    GetMem(Pixels, FontData.TexHeader.DataSize);
    //GetMem(Pixels, RoundBy(FontData.TexHeader.Width, 32) * RoundBy(FontData.TexHeader.Height, 32));
    GetMem(FontPixels, FontData.TexHeader.Width * FontData.TexHeader.Height);

    Stream.Read(Pixels^,  FontData.TexHeader.DataSize);
    //Stream.Read(FontPixels^,  FontData.TexHeader.DataSize);
    DecodeImage(Pixels, FontPixels, FontData.TexHeader.Width, FontData.TexHeader.Height);
    FreeMem(Pixels);

    FontData.MaxSize := 0;

    For n := 0 To FontData.FontHeader.Count - 1 do
    begin
      ...
      s := Rect.Bottom - Rect.Top + FontData.CharData[n].Y + 1;
      If w > s Then
        s := w;
      If s > FontData.MaxSize Then
        FontData.MaxSize := s;
      ...
      For y := Rect.Top To Rect.Bottom do
        ...
        Chars[n].Ch[y - Rect.Top + FontData.CharData[n].Y, 0], w);
    end;

    FreeMem(FontPixels);

    
    Section := FindSection(FontData, '_KERNINGHEADER_');
    Stream.Seek(Section^.Offset, 0);       
    Stream.Read(FontData.KerningHeader, SizeOf(TKerningHeader));
    //!!
    EndianKerningHeader(FontData.KerningHeader);


    SetLength(FontData.KerningData, FontData.KerningHeader.Count);
    Section := FindSection(FontData, '_KERNINGDATA_');              
    Stream.Read(FontData.KerningData[0], SizeOf(TKerningData) * FontData.KerningHeader.Count);

    //!!
    For n := 0 To FontData.KerningHeader.Count - 1 do
      //!!
  ...
  *)         
    Stream.Free;
    FontInit;
    FFileName := ChangeFileExt(FileName, '.drf');
    Saved := True;
