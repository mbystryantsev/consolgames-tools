...
Function LZ77Compress(Var Source, Dest; SrcLen: Integer; VRAMSafe: Boolean): Integer;
...
Function LZ_Compress
         (DestBuf: Pointer; Const SrcBuf: Pointer;
         SrcLen, VRAMsafe: Integer): Integer; stdcall;
         external 'GBALZ.DLL';
Function LZ_Decompress
         (DestBuf: Pointer; Const SrcBuf: Pointer): Integer; stdcall;
         external 'GBALZ.DLL';

Function LZ77Decompress(Var Source, Dest): Integer;
begin
 Result := LZ_Decompress(Addr(Dest), Addr(Source));
end;

Function LZ77Compress(Var Source, Dest; SrcLen: Integer; VRAMSafe: Boolean): Integer;
begin
 Result := LZ_Compress(Addr(Dest), Addr(Source), SrcLen, Integer(VRAMSafe));
end;
...
begin
 ...
 begin
  ...
  If not Zahuy and (NCount <= 255) then
  begin
   Tsz^ := NCount;
...
Procedure GetGBApalette(Var Source, Dest; Count: Integer);
Var SP: PWords; DP: PPalette; I: Integer;
begin
 SP := Addr(Source);
 DP := Addr(Dest);
 For I := 0 to Count - 1 do DP^[I] := GBA2Color(SP^[I]);
end;

Procedure SetGBApalette(Var Source, Dest; Count: Integer);
Var SP: PPalette; DP: PWords; I: Integer;
begin
 SP := Addr(Source);
 DP := Addr(Dest);
 For I := 0 to Count - 1 do DP^[I] := Color2GBA(SP^[I]);
end;
