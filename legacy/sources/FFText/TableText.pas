...
Type
  ...
  public
    ...
    Procedure AddItem(const Name: String = ''; const UserData: String = '';
              const Hidden: Boolean = False; const Caption: WideString = ''); // Добавить Item
...
var
  ...
  bCharIndex, bSetIndex: Boolean; CharsIndexes: TWordArray; CISize, CIStride: Integer;
...
const
  ...
  begin
    Case Mode of
      lnNormal: GetDataArray(FTable[Cur].Data, PWideChar(S), (P - 1) div 2);
      {begin
  ...
  begin
    ...
    begin
      ...
      Index := 0;
    SetLength(FTable[Cur].Indexes, (CISize + CIStride) div Size);
    For n := 0 To ((CISize + CIStride) div Size) - 1 do
    begin
      Case CISize of
...
begin
  ...
  If not Append Then Finalize(FTable);
  Fmt := False;
  _endian := False;
...
procedure TGameTextSet.AddItem(const Name: String; const UserData: String;
          const Hidden: Boolean; const Caption: WideString );
var Num: Integer;
begin
  Num := Length(FItems);
  SetLength(FItems, Num + 1);
  FItems[Num]          := TGameText.Create;
  FItems[Num].FName    := Name;
  FItems[Num].FParent  := Self;
  FItems[Num].FIndex   := Num;
  FItems[Num].UserData := UserData;
  FItems[Num].FHidden  := Hidden;  
  FItems[Num].FCaption := Caption;
end;
...
var P,E, n: Integer; Cur, Len, StrCount, Code, PreLen, PosLen, Pos: Integer; Info,Name,UserInfo: WideString; SCount: String;
begin
...
var n, L: Integer; B: Byte; D: TByteArray;
begin
  Result := 0;
  L := Len;
  ...
  begin
    If IsOp Then
    begin
      If ((Len=0) or (FOpcodeList.OpcodeLen(OpID)<=Len)) Then
      begin
...
var V,n,m, P, Cur, Cnt, ELen: Integer; Str: WideString; Mode: TDataMode;
 ...
 Function CompareStr(CS: WideString): Boolean;
 var n: Integer;
 begin
    Result := False;
    n := 1;
