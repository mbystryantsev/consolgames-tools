...
$Id: SynHighlighterSample.pas,v 1.6.2.11 2008/09/14 16:24:57 maelh Exp $
...
type
  ...
  private
    ...
    fByteArrti: TSynHighlighterAttributes;
    FTabsArrti: TSynHighlighterAttributes;
...
begin
  ...
  fByteArrti    := TSynHighLighterAttributes.Create('','Byte');
  fByteArrti.Style := [fsBold];
  fByteArrti.Foreground := clBlue;
  AddAttribute(fByteArrti);

  fTabsArrti    := TSynHighLighterAttributes.Create('','Tabs');
  fTabsArrti.Background := $00DDFEFF;
  AddAttribute(fTabsArrti);
...
begin
  ...
  case fRange of
    ...
    case fLine[Run] of
      ...
      #10: LFProc;
      #13: CRProc;
      '{': TagOpenProc;//BraceCommentOpenProc;
      //'/': CStyleCommentOpenProc;
      //'"': StringOpenProc;
      #1..#9, #11, #12, #14..#31: TabsProc;
      #32: SpaceProc;
      //'A'..'Z', 'a'..'z', '_': IdentProc;
    else
      UnknownProc;
...
begin
  case GetTokenID of
    ...
    tkByte: Result := fByteArrti;
    tkTab: Result := fTabsArrti;
  else
    Result := nil;
  end;
end;
...
procedure TSynFFSyn.TabsProc;
begin
  fTokenID := tkTab;
  repeat
    inc(Run);
  until not (fLine[Run] in [#1..#31]);
end;
...
begin
  ...
  repeat
    inc(Run);
  until not (fLine[Run] = #32);
end;

procedure TSynFFSyn.TagOpenProc;
var C: WideChar;
begin
  ...
  Case fLine[Run] of
    ...
    begin
      If (fLine[Run+2] = '}')
      and (fLine[Run+1] in ['0'..'9', 'A'..'F', 'a'..'f'])
      //and (fLine[Run+2] = '}')
      Then
      begin
        fRange := rsUnknown;
