...
type
 ...
 TVarWideStringStream = class(TStream)
  private
 ...
 TCustomUnpackStreamClass = class of TCustomUnpackStream;
...
procedure TStreamIniFile.LoadFromFile(const FileName: AnsiString);
var
 Stream: TFileStream;
begin
 Stream := TFileStream.Create(FileName, fmOpenRead or fmShareDenyWrite);
...
procedure TStreamIniFile.SaveToFile(const FileName: AnsiString);
var
 Stream: TFileStream;
begin
 Stream := TFileStream.Create(FileName, fmCreate);
...
begin
 Result := (TotalBitsCount shr 3) + Integer(Byte(TotalBitsCount) and 7 > 0);
end;
...
function TVarWideStringStream.GetSize: Int64;
begin
 Result := Length(FStringPtr^) shl 1;
end;
...
begin
 if (FPosition >= 0) and (Count >= 0) then
 begin
  Result := Length(FStringPtr^) shl 1 - FPosition;
  if Result > 0 then
  begin
   if Result > Count then Result := Count;
   System.Move(PByteArray(FStringPtr^)[FPosition], Buffer, Result);
   Inc(FPosition, Result);
   Exit;
  end;
 end;
...
function TVarWideStringStream.Seek(Offset: Integer; Origin: Word): Integer;
begin
 case Origin of
  soFromBeginning: FPosition := Offset;
  soFromCurrent: Inc(FPosition, Offset);
  soFromEnd: FPosition := Length(FStringPtr^) shl 1 + Offset;
 end;
 Result := FPosition;
end;
...
begin
 ...
 begin
  ...
  if Pos > 0 then
  begin
   if Pos > Length(FStringPtr^) shl 1 then SetLength(FStringPtr^, Pos shr 1);
