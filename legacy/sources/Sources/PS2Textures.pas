...
Procedure writeTexPSMT4(dbp, dbw, dsax, dsay, rrw, rrh: Integer; data: Pointer;
toGS: Boolean = True; Z: Boolean = False);
procedure writeTexPSMT8(dbp, dbw, dsax, dsay, rrw, rrh: Integer; data: Pointer; toGS: Boolean = True);
implementation
...
begin
  ...
  begin
    ...
    begin
      ...
      //page := ((y SHR 5) * (dbw SHR 6)) + (x SHR 6);   // It is right!
...
//    except
//      ShowMessage(Format('%dx%d, startBlockPos: %d, page: %d, block: %d, column: %d, cw: %d',
//      [x,y, startBlockPos, page, block, column, cw]));
//   end;
      Inc(src);
		end;
	end;
...
*)
  ...
  begin
    ...
    begin
			...
			pageY := y div 128;
			page  := pageX + pageY * dbw;
      //page := ((y shr 7) * ((dbw+127) shr 7)) + (x shr 7); // Xenosaga!!!
...
const
  block8: Array[0..32-1] of Integer =
  (
     0,  1,  4,  5, 16, 17, 20, 21,
     2,  3,  6,  7, 18, 19, 22, 23,
     8,  9, 12, 13, 24, 25, 28, 29,
  ...
  columnWord8: Array[0..2-1, 0..64-1] of Integer =
  (
    (
  ...
  columnByte8: Array[0..64-1] of Integer =
  (
...
var
    startBlockPos, x, y, pageX, pageY, page, px, py, blockX, blockY, block,
    column, cx, cy,cw, bx,by, cb      : Integer;
    src, dst: ^Byte;
begin
	dbw := dbw SHR 1;
	src := data;
  ...
  begin
    ...
    begin
      ...
      by := py - (blockY * 16);
			...
			cx := bx;
			cy := by - column * 4;
			cw := columnWord8[column AND 1, cx + cy * 16];
			cb := columnByte8[cx + cy * 16];

			dst := @gsmem[startBlockPos + page * 2048 + block * 64 + column * 16 + cw];
      If toGS Then    
			  Byte(Pointer(DWord(dst)+cb)^) := src^
      else              
        src^ := Byte(Pointer(DWord(dst)+cb)^);
      Inc(src);
		end;
	end;
end;
