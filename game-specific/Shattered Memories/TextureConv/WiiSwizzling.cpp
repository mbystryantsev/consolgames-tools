...
static void swapDecode32(void* data, int width, int height, const char* map)
{
	...
	uint16_t* a = static_cast<uint16_t*>(data);
...
static void swapEncode32(void* data, int width, int height)
{
	...
	for (int i = 0; i < width * height; i+=16)
	{
		...
		{
			uint16_t* b = a + 16;
			uint16_t v[8] = {a[0], a[1], a[2], a[3], b[0], b[1], b[2], b[3]};
...
{
	swizzleProc<typeUnswizzle, 8, 4>(static_cast<const uint8_t*>(src), static_cast<uint8_t*>(dst), width * 4, height);
	swapDecode32(dst, width, height);
}

void wiiSwizzle32(const void* src, void* dst, int width, int height)
{
	swizzleProc<typeSwizzle, 8, 4>(static_cast<uint8_t*>(dst), static_cast<const uint8_t*>(src), width * 4, height);
	swapEncode32(dst, width, height);
}

void wiiUnswizzle16(const void* src, void* dst, int width, int height)
{
	swizzleProc<typeUnswizzle, 8, 4>(static_cast<const uint8_t*>(src), static_cast<uint8_t*>(dst), width * 2, height);
}

void wiiSwizzle16(const void* src, void* dst, int width, int height)
{
	swizzleProc<typeSwizzle, 8, 4>(static_cast<uint8_t*>(dst), static_cast<const uint8_t*>(src), width * 2, height);
}

void wiiUnswizzle8(const void* src, void* dst, int width, int height)
{
	swizzleProc<typeUnswizzle, 8, 4>(static_cast<const uint8_t*>(src), static_cast<uint8_t*>(dst), width, height);
}

void wiiSwizzle8(const void* src, void* dst, int width, int height)
{
	swizzleProc<typeSwizzle, 8, 4>(static_cast<uint8_t*>(dst), static_cast<const uint8_t*>(src), width, height);
}

void wiiUnswizzle4(const void* src, void* dst, int width, int height)
{
	swizzleProc<typeUnswizzle, 4, 8>(static_cast<const uint8_t*>(src), static_cast<uint8_t*>(dst), width / 2, height);
}

void wiiSwizzle4(const void* src, void* dst, int width, int height)
{
	swizzleProc<typeSwizzle, 4, 8>(static_cast<uint8_t*>(dst), static_cast<const uint8_t*>(src), width / 2, height);
}